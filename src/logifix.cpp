...
std::string
apply_rewrite(const std::string& original,
    ...
    for (const auto& x : after) {
        ...
        for (const auto& y : before) {
            ...
            if (std::get<0>(y) <= std::get<0>(x)) {
                diff += std::get<2>(y).size() -
                        int(std::get<1>(y) - std::get<0>(y));
            }
        }
        result.emplace_back(std::get<0>(x) + diff, std::get<1>(x) + diff,
                            std::get<2>(x));
    }
    return result;
}

rewrite_collection rewrites_invert(const std::string& original,
...
bool rewrite_collections_overlap(const rewrite_collection& left,
                                 const rewrite_collection& right) {
...
bool rewrite_collection_overlap(const rewrite_collection& coll) {
    for (size_t i = 0; i < coll.size(); i++) {
        for (size_t j = 0; j < coll.size(); j++) {
            if (i == j)
                continue;
            if (segments_overlap<size_t>(
                    {std::get<0>(coll[i]), std::get<1>(coll[i])},
...
split_rewrite(const std::string& original,
              const std::tuple<size_t, size_t, std::string>& rewrite) {
...
get_patches_for_file(node_id node) {
    ...
    for (auto [rule_id, node_id] : taken_transitions[node]) {
        auto result = post_process(
            original, get_recursive_merge_result_for_node(node_id));
        ...
        thread_pool.emplace_back(std::thread([&] {
            while (true) {
                ...
                /* acquire work */
                {
                    ...
                    std::cerr << pending_strings.size() << " "
                              << pending_files.front() << std::endl;
                ...
                for (auto& [next_node, rule, take_transition] : next_nodes) {
                    ...
                    std::cerr << "CHECKING NODE " << next_node << " " << rule
                              << std::endl;
                    std::unique_lock<std::mutex> lock(work_mutex);
                    bool current_has_parent =
                        parent.find(current_node) != parent.end();
                    if (current_has_parent) {
                        ...
                        auto inverted =
                            rewrites_invert(curr_pstr, curr_rewrites);
                        auto adjusted =
                            adjust_rewrites(inverted, next_rewrites);
                        if (!rewrite_collections_overlap(inverted,
                                                         next_rewrites)) {
                            auto candidate_string =
                                apply_rewrites(curr_pstr, adjusted);
                ...
                if (current_has_parent) {
                    ...
                    if (!rewrites.empty()) {
                        if (rewrite_collection_overlap(rewrites)) {
                            ...
                            auto next_node =
                                create_id(current_node_source, rewrites);
                            parent[next_node] = {rule, current_node};
...
get_patches(const std::string& source) {
    ...
    std::set<std::pair<std::string, std::tuple<size_t, size_t, std::string>>>
        rewrites;
